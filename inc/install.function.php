...
function check_directories() {
   ...
   if (!is_writable($go_dir.'inc/') 
         || !is_writable($go_dir.'front/') || !is_writable($go_dir.'ajax/')) {
...
function plugin_genericobject_install() {
      ...
      $query = "INSERT INTO `glpi_displaypreferences` (`id`, `itemtype`, `num`, `rank`, `users_id`) " .
               "VALUES (NULL, 4850, 10, 6, 0), (NULL, 4850, 9, 5, 0), (NULL, 4850, 8, 4, 0),
                       ...
                       (NULL, 4090, 4, 1, 0), (NULL, 4850, 11, 7, 0), (NULL, 4850, 12, 8, 0),
                        (NULL, 4850, 13, 9, 0), (NULL, 4850, 14, 10, 0), (NULL, 4850, 15, 11, 0);";
      $DB->query($query);
   ...
   if (!FieldExists('glpi_plugin_genericobject_types', 'use_network_ports')) {
      $DB->query("ALTER TABLE `glpi_plugin_genericobject_types` " .
                 "ADD `use_network_ports` INT( 1 ) NOT NULL DEFAULT '0'");
   }

   if (!FieldExists('glpi_plugin_genericobject_types', 'use_direct_connections')) {
      $DB->query("ALTER TABLE `glpi_plugin_genericobject_types` " .
                 "ADD `use_direct_connections` INT( 1 ) NOT NULL DEFAULT '0'");
   }

   if (!FieldExists('glpi_plugin_genericobject_types', 'use_plugin_geninventorynumber')) {
      $DB->query("ALTER TABLE `glpi_plugin_genericobject_types` " .
                 "ADD `use_plugin_geninventorynumber` INT( 1 ) NOT NULL DEFAULT '0'");
   }
...
function plugin_genericobject_uninstall() {
   ...
   $tables = array ("glpi_plugin_genericobject_types", "glpi_plugin_genericobject_profiles",
                    "glpi_plugin_genericobject_type_fields", "glpi_plugin_genericobject_type_links");
